...
local function LogFire(desc, noMount)
    ...
    if not mapID then
        print("|cffFF0000CozyCamps:|r No map ID.")
        return
    end

    local pos = C_Map.GetPlayerMapPosition(mapID, "player")
    if not pos then
        print("|cffFF0000CozyCamps:|r No position.")
        return
    end
    ...
    if overlapCheckEnabled and CozyCampsLoggedFires[zone] then
        for _, existing in ipairs(CozyCampsLoggedFires[zone]) do
            ...
            if dist < OVERLAP_MAP_UNITS then
                ...
                print(string.format("|cffFFAA00CozyCamps:|r Fire already logged %.1f yards away.", distYards))
                return
            end
        end
    end
    ...
    CozyCampsLoggedFires[zone] = CozyCampsLoggedFires[zone] or {}
    table.insert(CozyCampsLoggedFires[zone], entry)
    ...
    print("|cff00FF00CozyCamps:|r Fire logged!" .. mountStatus)
...
local function Export()
    ...
    for z in pairs(CozyCampsLoggedFires) do
        table.insert(zones, z)
    end
    table.sort(zones)

    print("|cff88CCFF=== CozyCamps Fire Export ===|r")
    print("-- Copy this to FireDB.lua --")

    for _, z in ipairs(zones) do
        local fires = CozyCampsLoggedFires[z]
...
SlashCmdList["LOGFIRE"] = function(msg)
    ...
    elseif cmd == "clear" then
        CozyCampsLoggedFires = {}
        print("|cff88CCFFCozyCamps:|r Fire log cleared.")

    elseif cmd == "count" then
        local c = 0
        for _, fires in pairs(CozyCampsLoggedFires) do
            c = c + (fires and #fires or 0)
        end
        print(string.format("|cff88CCFFCozyCamps:|r %d fires logged.", c))
    ...
    elseif cmd == "overlap" then
        ...
        print("|cff88CCFFCozyCamps:|r Overlap checking " ..
