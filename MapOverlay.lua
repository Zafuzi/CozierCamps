...
-- Icon textures
local FIRE_ICON    = "Interface\\AddOns\\CozyCamps\\assets\\fireicon"
local INN_ICON     = "Interface\\AddOns\\CozyCamps\\assets\\exhaustionicon"
local TRAINER_ICON = "Interface\\AddOns\\CozyCamps\\assets\\anguishicon"
local COOKING_ICON = "Interface\\AddOns\\CozyCamps\\assets\\hungericon"
...
local function ShouldShowForFaction(locationFaction)
    if locationFaction == FACTION_NEUTRAL then return true end
...
local function AcquirePin(pool, creator)
    ...
    if not pin then pin = creator() end
    return pin
end
...
local function ClearAllPins()
    for _, pin in ipairs(activeFirePins) do ReleasePin(firePinPool, pin) end
    for _, pin in ipairs(activeInnPins) do ReleasePin(innPinPool, pin) end
    for _, pin in ipairs(activeTrainerPins) do ReleasePin(trainerPinPool, pin) end
    for _, pin in ipairs(activeCookingPins) do ReleasePin(cookingPinPool, pin) end
...
local function UpdateMapPins()
    ...
    if not mapInfo or not mapInfo.name then return end
...
local function CreateMapCheckbox()
    if mapCheckbox or not WorldMapFrame then return end

    local parent = WorldMapFrame.BorderFrame or WorldMapFrame
    mapCheckbox = CreateFrame("CheckButton", "CozyCampsSurvivalIconsCheckbox", parent, "UICheckButtonTemplate")
...
local function UpdateCheckboxVisibility()
    if not mapCheckbox then return end
...
watcher:SetScript("OnUpdate", function(_, elapsed)
    ...
    if not GetSetting("showSurvivalIcons", false) then return end
