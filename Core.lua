...
CC.ASSET_PATH = "Interface\\AddOns\\CozyCamps\\assets\\"
...
local function InitializeSavedVariables()
    if not CozyCampsDB then
        CozyCampsDB = {}
    end
    for key, default in pairs(DEFAULT_SETTINGS) do
        if CozyCampsDB[key] == nil then
            CozyCampsDB[key] = default
        end
    end
    CC.db = CozyCampsDB

    -- Initialize character-specific saved variables
    if not CozyCampsCharDB then
        CozyCampsCharDB = {}
    end
    CC.charDB = CozyCampsCharDB
end
...
function CC.GetFireLocations(zoneName)
    if not CozyCampsFireDB then
        return nil
    end
    return CozyCampsFireDB[zoneName]
end
...
local function CheckUHCConflict()
    ...
    if hasConflict then
        local msg = "|cffFF6600CozyCamps Warning:|r UltraHardcore has conflicting settings enabled:\n"
        for _, c in ipairs(conflicts) do
            msg = msg .. "  - " .. c .. "\n"
        end
        msg = msg .. "Please disable these in UHC or CozyCamps to avoid issues."
...
local function CreateLevelRequirementPopup()
    local popup = CreateFrame("Frame", "CozyCampsLevelPopup", UIParent, "BackdropTemplate")
    ...
    text:SetWidth(320)
    text:SetText(
        "CozyCamps survival systems will activate once you reach |cffFF9933Level 6|r.\n\nUntil then, enjoy your early adventures!")
...
local function ShowLevelRequirementInfo()
    ...
    -- Show chat message
    print("|cff88CCFFCozyCamps:|r Survival systems will activate at |cffffd700Level " .. MIN_LEVEL ..
...
local function OnLevelUp(newLevel)
    if newLevel == MIN_LEVEL then
        print("|cff88CCFFCozyCamps:|r |cff00ff00You've reached Level " .. MIN_LEVEL ..
                  "! Survival systems are now active.|r")
        print("|cff88CCFFCozyCamps:|r Type |cffffff00/cozy|r to open settings.")
    end
end
...
initFrame:SetScript("OnEvent", function(self, event, arg1)
    if event == "ADDON_LOADED" and arg1 == "CozyCamps" then
        InitializeSavedVariables()
    elseif event == "PLAYER_LOGIN" then
        print("|cff88CCFFCozyCamps|r v" .. CC.version .. " loaded. Type |cffffff00/cozy|r for commands.")
        CheckUHCConflict()
...
SlashCmdList["REST"] = function(msg)
    if CC.GetSetting("fireDetectionMode") == 2 then
        ...
        print("|cff88CCFFCozyCamps:|r /rest only works in Manual Rest Mode. Change mode in settings.")
    end
end
...
SlashCmdList["COZYCAMP"] = function(msg)
    ...
    if command == "" then
        if CC.OpenSettings then
            ...
            print("|cff88CCFFCozyCamps:|r Settings UI not available.")
        end
        return
    elseif command == "status" or command == "s" then
        ...
        print("|cff88CCFFCozyCamps Status:|r")
    ...
    elseif command == "debug" or command == "debugpanel" or command == "dp" or command == "sliders" then
        if CC.ToggleDebugPanel then
            ...
            print("|cff88CCFFCozyCamps:|r Debug panel not available")
        end
        return
    elseif command == "proximity" then
        ...
        print("|cff88CCFFCozyCamps:|r Proximity debug " .. (not current and "|cff00FF00ON|r" or "|cffFF0000OFF|r"))
    ...
    elseif command == "exhaustion" then
        ...
        print("|cff88CCFFCozyCamps:|r Exhaustion debug " .. (not current and "|cff00FF00ON|r" or "|cffFF0000OFF|r"))

    elseif command == "Anguish" then
        local current = CC.GetSetting("AnguishDebugEnabled")
        CC.SetSetting("AnguishDebugEnabled", not current)
        print("|cff88CCFFCozyCamps:|r Anguish debug " .. (not current and "|cff00FF00ON|r" or "|cffFF0000OFF|r"))
    ...
    elseif command == "temperature" or command == "temp" then
        ...
        print("|cff88CCFFCozyCamps:|r Temperature debug " .. (not current and "|cff00FF00ON|r" or "|cffFF0000OFF|r"))
    ...
    elseif command == "mode" or command == "displaymode" then
        ...
        print("|cff88CCFFCozyCamps:|r Meter display mode set to |cffFFD700" .. newMode .. "|r")
        print("|cff88CCFFCozyCamps:|r |cffFFFF00/reload required to apply changes|r")

    elseif command == "bar" then
        CC.SetSetting("meterDisplayMode", "bar")
        print("|cff88CCFFCozyCamps:|r Meter display mode set to |cffFFD700bar|r")
        print("|cff88CCFFCozyCamps:|r |cffFFFF00/reload required to apply changes|r")

    elseif command == "vial" then
        CC.SetSetting("meterDisplayMode", "vial")
        print("|cff88CCFFCozyCamps:|r Meter display mode set to |cffFFD700vial|r")
        print("|cff88CCFFCozyCamps:|r |cffFFFF00/reload required to apply changes|r")
    ...
    elseif command == "help" or command == "?" or command == "" then
        print("|cff88CCFF=== CozyCamps v" .. CC.version .. " ===|r")
        print("|cffffff00/cozy|r or |cffffff00/cozycamps|r - Open config page")
        print("|cffffff00/rest|r - Activate rest (Manual Rest Mode only)")
        print("|cffffff00/logfire [desc]|r - Log fire at current position")

    else
        print("|cff88CCFFCozyCamps:|r Unknown command. Use |cffffff00/cozy|r for help.")
    end
end

-- Compatibility alias
CookingRangeCheck = CozyCamps
