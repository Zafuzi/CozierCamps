-- CozierCamps - Anguish.lua
-- Anguish system (OnUpdate-only, no RegisterEvent) - Midnight 12.0.1 / 120001 SAFE
--
-- FIX: Some UIs/taint states are blocking ANY Frame:RegisterEvent() from CozierCamps, producing:
-- [ADDON_ACTION_FORBIDDEN] AddOn 'CozierCamps' tried to call the protected function 'Frame:RegisterEvent()'.
...
local function IsSafeNumber(v)
	...
	return pcall(function() local _ = v + 0 return _
	end)
end
...
local function CoerceSafeNumber(v)
	...
	local n
	local ok = pcall(function() n = tonumber(v)
	end)
	if not ok then
		return nil
	end
...
local OVERLAY_TEXTURES = {
	"Interface\\AddOns\\CozierCamps\\assets\\anguish20.png",
	"Interface\\AddOns\\CozierCamps\\assets\\anguish40.png",
	"Interface\\AddOns\\CozierCamps\\assets\\anguish60.png",
	"Interface\\AddOns\\CozierCamps\\assets\\anguish80.png"
}
...
------------------------------------------------------------
function CC.GetAnguish() return anguish
end
function CC.GetAnguishPercent() return anguish / MAX_ANGUISH
end
