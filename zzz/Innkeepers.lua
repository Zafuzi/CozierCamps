...
-- Method 1: Try C_TooltipInfo API (modern retail)
	...
	if not CozierCampsInnkeeperScanTooltip then
		CozierCampsInnkeeperScanTooltip = CreateFrame("GameTooltip", "CozierCampsInnkeeperScanTooltip", nil, "GameTooltipTemplate")
		CozierCampsInnkeeperScanTooltip:SetOwner(WorldFrame, "ANCHOR_NONE")
	end

	CozierCampsInnkeeperScanTooltip:ClearLines()
	CozierCampsInnkeeperScanTooltip:SetUnit("npc")

	local numLines = CozierCampsInnkeeperScanTooltip:NumLines()
	if numLines >= 2 then
		local line2 = _G["CozierCampsInnkeeperScanTooltipTextLeft2"]
		if line2 then
			local text = line2:GetText()
			if text and text ~= "" then
				return text
