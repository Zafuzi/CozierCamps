-- CozyCamps - FirstAidTrainers.lua
...
local function GetNpcSubtitle()
    ...
    if not CozyCampsScanTooltip then
        CozyCampsScanTooltip = CreateFrame("GameTooltip", "CozyCampsScanTooltip", nil, "GameTooltipTemplate")
        CozyCampsScanTooltip:SetOwner(WorldFrame, "ANCHOR_NONE")
    end

    CozyCampsScanTooltip:ClearLines()
    CozyCampsScanTooltip:SetUnit("npc")

    -- Check line 2 for subtitle
    local numLines = CozyCampsScanTooltip:NumLines()
    Debug("GetNpcSubtitle: Scan tooltip has " .. tostring(numLines) .. " lines")
    if numLines >= 2 then
        local line2 = _G["CozyCampsScanTooltipTextLeft2"]
        if line2 then
            local text = line2:GetText()
            if text and text ~= "" then
